...
PropertyDefinition is the standard javascript property definition. see https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object/defineProperty
 see below DbObjectType.create.
...
DbObjectType.prototype = {
        ...
        propertyNames.forEach(function(propertyName) {
            ...
            case 'object':
                if (propertyValue != null) {
                    var fnName = 'toJSONValue';
                    var specialHandlingForProperty = _.deep(specialHandling, propertyName);
                    var jsonFn = _.deep(specialHandlingForProperty, fnName);
                    if (!jsonFn) {
                        var jsonHelperObj = _.deep(specialHandlingForProperty, 'jsonHelper');
                        if (jsonHelperObj != null) {
                            var jsonHelper;
                            if ( typeof jsonHelperObj === 'string' ) {
                                jsonHelper = _getJSONHelper(jsonHelperObj);
                                if ( jsonHelper == null ) {
                                    debugger;
                                    throw new Meteor.Error(500, fnName+" "+self.typeName()+'.'+propertyName+
                                        '.jsonHelper='+jsonHelperObj+
                                        " which isn't a global var name or registered jsonHelper"
                                    );
                                }
                            } else {
                                jsonHelper = jsonHelperObj;
                            }
                            if (typeof jsonHelper[fnName] === 'function') {
                                jsonFn = jsonHelper[fnName];
                            }
                        }
                    }
                    if (jsonFn) {
     ...
     * values allowed from the client are updated.
     *
     ...
     *
     * 'clientObj' - new values to upsert
     * 'lookup' - (optional) either mongodb query to find existing or a string which is the object id
     * or object (default={_id: clientObj._id}) or function yielding existing object.
     ...
     * 'revision' - use _revisionSave rather than _save
     *
...
var __findFnArguments = function(propertyName, value) {
    ...
    var args = [request].concat(Array.prototype.slice.call(arguments, 2));
    return args;
}
