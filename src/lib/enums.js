 ...
 *
 * NOTE: "This pattern" refers to the (function(exports) { ... }) bit.
 *
...
Enum.Esymbol have a asMongoDb() function that returns a { <typeName>: <dbCode> } - this is suitable for querying for a certain value in the db.
    For Example,
    UploadStatus.done.asMongoDb() ==> { 'uploadStatus': 'done' }
    UploadStatus.done.asMongoDb('different') ==> { 'different': 'done' }
...
(function (exports) {
    ...
    Esymbol.prototype = Object.create(null);
    Esymbol.prototype.constructor = Esymbol;
    /**
     * Without Object.prototype in the prototype chain, we need toString()
     * in order to display symbols. 
     * Unfortunately, javascript uses the toString() value if the Esymbol is used as an object
     * index. For example:
     ...
     */
    Esymbol.prototype.toString = function () {
        if ( this.dbCode ) {
            return this.dbCode;
        } else {
            return this.name;
    ...
    Esymbol.prototype.hasOwnProperty = function(propertyName) {
        return this.propertyNames.indexOf(propertyName) > -1;
    }
    Esymbol.prototype.toJSONValue = Esymbol.prototype.toJSON = function() {
        if ( this.dbCode ) {
            return this.dbCode;
        } else {
            return this.name;
    ...
    Object.defineProperties(Esymbol.prototype, {
    ...
    Object.freeze(Esymbol.prototype);
    ...
    var Enum = function (options) {
        ...
        function __createSymbol(name, symbolDefinition) {
            ...
            var symbol = new Esymbol(name, symbolDefinition);
            ...
            Object.defineProperties(symbol, {
                ...
                inMongoDb: {
                    ...
                    /**
                     * May be null ( use the typeName )
                     * wrapper function for Enums.inMongoDb
                     * @param dbField
                     * @returns {*}
                     */
                    value: function () {
        ...
        // === State Machine Code ===
        var stateMachineEnum = false;
        symbols.forEach( function(symbol) {
            if ( symbol.returnAllowed !== false && symbol.nextStates instanceof Array) {
                stateMachineEnum = true;
                symbol.nextStates.forEach(function (toSymbol) {
                    if (toSymbol.backStates == null) {
                        toSymbol.backStates = [];
                    }
                    toSymbol.backStates.push(symbol);
                });
            }
        });
        if ( stateMachineEnum ) {
            symbols.forEach(function (symbol) {
                if (symbol.nextStates != null) {
                    symbol.nextStates = Object.freeze(symbol.nextStates);
                } else {
                    symbol.nextStates = Object.freeze([]);
                }
                symbol.terminal = symbol.nextStates.length == 0;
                if (symbol.backStates != null) {
                    symbol.backStates = Object.freeze(symbol.backStates);
                } else {
                    symbol.backStates = Object.freeze([]);
                }
            });
        }
        // === end state machine ===
        ...
        //
        // after the dev has chance to alter the symbols and enum - freeze the symbols
        symbols.forEach(function(symbol) {
    ...
    _.extend(Enum.prototype, {
        ...
        contains: function (sym) {
            ...
            } else if (sym instanceof Esymbol) {
        ...
        /**
         * Convert an array of dbCode values to an array of Esymbol objects. Useful for deseralization.
         * @param inputArray
         * @returns {Array}
         */
         ...
         * @return { $nin: [ <dbCodes> ] } if multiple or { $ne: dbCode } if single argument
         */
        ninMongoDb: function() {
            var enums, fieldName;
            if ( _.isString(arguments[0]) ) {
